...
def process_viirs_filename(file_path: Path) -> Tuple[str, str]:
    ...
    pattern = r"\d{7}"  # reg expression pattern for 7 consecutive digits
    ...
    if match:
        ...
        print("error, no date str in process_viirs_filename")

    dateStrFormat = datetime.strptime(dateStr, "%Y%j").strftime(
        "%Y-%m-%d"
    )  # return val for date

    endInd = pathStr.find(dateStr) + 7  # get end of digit
    if pathStr[endInd] != ".":
...
def process_s1_filename(file_path: Path) -> Tuple[str, str]:
    ...
    """
    pathStr = file_path.as_posix()
    pattern = r"\d{8}"  # reg expression pattern for 7 consecutive digits
    match = re.search(pattern, pathStr)
    ...
    if match:
        ...
        print("error, no date str in process_s1_filename")

    dateStrFormat = datetime.strptime(dateStr, "%Y%m%d").strftime(
        "%Y-%m-%d"
    )  # return val for date

    endInd = pathStr.find(dateStr) + 8  # get end of digit
    if pathStr[endInd] != ".":
        return [dateStrFormat, pathStr[endInd + 1 : pathStr.rfind(".")]]
    else:
        return [dateStrFormat, "0"]
...
def process_s2_filename(file_path: Path) -> Tuple[str, str]:
    ...
    if match:
        ...
        print("error, no date str in process_s2_filename")
...
def process_landsat_filename(file_path: Path) -> Tuple[str, str]:
    ...
    """
    pathStr = file_path.as_posix()
    pattern = r"\d{4}"  # reg expression pattern for 4 consecutive digits
    match = re.search(pattern, pathStr)
    ...
    if match:
        ...
        print("error, no date str in process_landsat_filename")

    endInd = dateInd + 10  # get end of date
    if pathStr[endInd] != ".":
...
def get_unique_dates_and_bands(
    ...
    satType = list(sorted(satType))

    return dates, satType
...
def get_parent_tile_id(tile_dir: Path) -> str:
    ...
    """
    dirStr = str(tile_dir)
    parentTileId = dirStr[dirStr.rfind("/") + 1 :]
    return parentTileId
...
def read_satellite_file(satellite_file: Path) -> np.ndarray:
    ...
    """
    try:
        image_data = tifffile.imread(str(satellite_file))
        image_data = image_data.astype(np.float32)

        return image_data
    except:
        print("file name w/error:", str(satellite_file))
        return
...
def load_satellite(tile_dir: Path, satellite_type: SatelliteType) -> xr.DataArray:
        ...
        for band in bands:
            ...
            for fileName in file_names:
                ...
                if date == retDate and band == retBand:
                    ...
                    # break ??? SHOULD WE ACTUALLY BREAK?
    ...
    height, width = finData.shape[-2:]  # image height and width
    coords = {
        "date": dates,
        "band": bands,
        "height": range(height),
    ...
    xrArr = xr.DataArray(
        finData, dims=("date", "band", "height", "width"), coords=coords
    )
    ...
    print("satellite_type: bla bla", satellite_type.value)
...
def load_satellite_list(
    ...
    """
    loadedSats = []
    for satType in satellite_type_list:
        loadedSats.append(load_satellite(tile_dir, satType))
