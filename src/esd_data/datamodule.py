...
class ESDDataModule(pl.LightningDataModule):
    ...
    def prepare_data(self) -> None:
        ...
        if not self.processed_dir.exists() or not list(self.processed_dir.glob("*")):
            ...
            for tileDir in tqdm(list(tile_dirs_train), desc="Processing train tiles"):
                # get the data array list and gt data array from load_and_preprocess
                dataArrList, gtArrData = self.load_and_preprocess(tileDir)
                # create a subtile, passing the data array list, gt data array, and the slice size
                subtileInst = Subtile(dataArrList, gtArrData, self.slice_size)
                # save the subtile to the train_dir
                subtileInst.save(self.train_dir)
    ...
    def setup(self, stage: str) -> None:
        ...
        if stage == "test":
            # create the test ESDDataset (the processed_dir will be the train_dir)
    ...
    def train_dataloader(self) -> torch.utils.data.DataLoader:
        ...
        return torch.utils.data.DataLoader(
            self.train_dataset, batch_size=self.batch_size, collate_fn=collate_fn, num_workers=self.num_workers
        )
    ...
    def val_dataloader(self) -> torch.utils.data.DataLoader:
        ...
        # and collate_fn
        return torch.utils.data.DataLoader(
            self.val_dataset, batch_size=self.batch_size, collate_fn=collate_fn, num_workers=self.num_workers
        )
