...
class ESDDataset(Dataset):
    ...
    def __init__(
        ...
        for tiles in processed_dir.iterdir():
            # iterate over the subtiles within the tile
            for subtiles in tiles.iterdir():
                for subtile in subtiles.iterdir():
    ...
    def __aggregate_time(self, array: np.ndarray):
        ...
        newArr = array.reshape(array.shape[0] * array.shape[1], array.shape[2], array.shape[3])
        return newArr
